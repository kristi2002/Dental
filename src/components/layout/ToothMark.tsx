 ...
 *  Takes its colour from `className` so it can sit on the teal masthead as
 *  easily as on a white page. */
export function ToothMark({ size = 34, className }: { size?: number; className?: string }) {
  return (
    <svg
      width={size}
      height={size}
      ...
      fill="none"
      aria-hidden
      className={cn('shrink-0', className ?? 'text-brand')}
    >
      <path
