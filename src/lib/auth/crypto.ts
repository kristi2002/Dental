...
/**
 * Random numeric PIN, for whoever resets someone's access.
 *
 ...
 *
 * **No caller yet.** `staff.ts` takes a PIN typed into a form, so nothing in the
 * application reaches this — it was deleted once as dead and restored
 * deliberately, because the reasoning above is the expensive part and a reset
 * flow that has to re-derive it is a reset flow that will use `byte % 10`.
 * `crypto.test.ts` holds it honest in the meantime; wire it up rather than
