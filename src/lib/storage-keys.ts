 ...
 * referencing this?", because a sweeper that knows about three of the four
 * columns does not report a smaller number: it **deletes the fourth column's
 ...
 * on the first `--apply` — silently, and an hour after upload.
 *
 ...
 * holds a storage key is missing from it. Adding a fifth place to store a file
 * now breaks a test instead of eating the fourth.
 */

/**
 * The shape of the client this needs — the four finders, and nothing else.
 *
 ...
 * directory, and four narrow queries against a four-person practice is cheaper
...
export async function referencedStorageKeys(prisma: StorageKeyReader): Promise<Set<string>> {
  const [documents, attachments, items, products] = await Promise.all([
