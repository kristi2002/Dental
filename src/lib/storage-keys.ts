 ...
 * holds a storage key is missing from it. Adding a sixth place to store a file
 * now breaks a test instead of eating the fifth.
 */

/**
 * The shape of the client this needs — the five finders, and nothing else.
 *
 ...
 * directory, and five narrow queries against a four-person practice is cheaper
...
export async function referencedStorageKeys(prisma: StorageKeyReader): Promise<Set<string>> {
  const [documents, attachments, mail, items, products] = await Promise.all([
