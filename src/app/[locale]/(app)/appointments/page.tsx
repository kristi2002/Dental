...
export default async function AppointmentsPage({
  ...
  const staffFilter = providers.some((person) => person.id === rawStaff)
    ? rawStaff!
    : "";
  ...
  const [
    ...
    canSeeWaitlist
      ? findFreeGaps({ date: anchor, staffUserId: staffFilter })
      : [],
    ...
    view === "week"
      ? Promise.all(
          weekDays(anchor).map((day) => getDaySchedule(day, staffFilter)),
        )
      : Promise.resolve([]),
  ...
  return (
    <>
      ...
      {/* The month rail sits beside the schedule on a desktop and under it on a
          ...
          It sits on the RIGHT: the left of the window is the navigation rail,
          and two columns of chrome down the same edge would have pushed the
          week grid 500px off the side of the screen. The split is honest as
          well as tidy — left takes you to another screen, right changes what
          this one is showing. */}
          ...
          <div className="mb-4 flex flex-wrap items-center gap-x-3 gap-y-2">
            <div className="flex items-center gap-1">
              ...
              {/* Date and opening hours as one centred block, so the chevrons
                  ...
                  the column is wide enough to spend it; below that the date is
                  allowed to take a second line rather than push the arrows out
                  past the calendar. */}
              <div className="min-w-0 px-1 text-center xl:min-w-[15rem]">
                <p className="text-[1.15rem] font-semibold leading-tight text-ink">
                  {label}
                </p>
                {view === "day" ? (
              ...
              <Link
                href={hrefFor(today())}
                className="btn btn-secondary btn-sm ml-1 shrink-0"
              >
                {t("today")}
              </Link>
            </div>
          ...
          {providers.length > 1 ? (
            ...
            >
              <Link
                ...
                className={cn(
                  "btn btn-sm",
                  staffFilter ? "btn-secondary" : "btn-primary",
                )}
              >
                {t("allProviders")}
              </Link>
