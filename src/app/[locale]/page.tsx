import { CalendarDays, Package, TriangleAlert, Users } from 'lucide-react';
...
export default async function DashboardPage({
  ...
  const [todayAppointments, weekCount, patientCount, lowStock, patients, services] =
    await Promise.all([
  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2">
        <PatientFormDialog triggerClassName="btn btn-primary btn-lg w-full" />
          ...
          {lowStock.length === 0 ? (
            ...
            <ul>
              {lowStock.map((item) => (
                <li
                  key={item.id}
                  className="flex items-center justify-between gap-3 border-b-2 border-line px-5 py-3.5 last:border-b-0"
                >
